...
export default function VendorGalleryPage() {
      ...
      try {
        ...
        if (vendorData) {
          ...
          toast({
            ...
            description: "Please create your vendor profile first",
          })
  ...
  const handleCoverImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Validate file size (2MB limit for base64 approach)
    ...
    try {
      ...
      if (coverImage) {
        try {
          ...
          await uploadCoverImageToFirestore(vendorId, coverImage, (progress) => {
      ...
      if (galleryImages.length > 0) {
        try {
          ...
          // Use the new Firestore method instead of Firebase Storage
          await uploadGalleryImagesToFirestore(vendorId, galleryImages, (progress) => {
